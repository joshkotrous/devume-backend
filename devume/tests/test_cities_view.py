...
class CitiesViewTestCase(APITestCase):
    ...
    def test_create_city(self):
        headers = {"x-api-key": str(self.apikey.key)}
        city_data = {"name": "Albany", "state": self.state.id}
        response = self.client.post(
            "/api/cities/create", city_data, format="json", headers=headers
        )
    ...
    def test_update_city(self):
        ...
        headers = {"x-api-key": str(self.apikey.key)}
        city_data = {"name": "Rochester", "state": self.state.id}
        response = self.client.patch(
            f"/api/cities/{self.city.id}/update",
            city_data,
            format="json",
            headers=headers,
        )
